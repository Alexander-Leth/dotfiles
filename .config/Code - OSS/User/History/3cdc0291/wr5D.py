...
class CrudEntity:
    ...
    def update_entity(session: Session, entity_id: int, entity_update: CreateAndUpdateCar): -> CarInfo:
...
class CrudSleep:
    ...
    def log_sleep(db: Session, sleep_data: schemas.LogSleep): -> SleepData:
        db_sleep = session.query(SleepData).(SleepData.date == db_sleep.date,
        ...
        SleepData.wakeCount = db_sleep.wakeCount).first()
