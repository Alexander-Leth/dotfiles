...
def get_car_info(entity_id: int, session: Session = Depends(get_db)):
...
def update_car(entity_id: int, new_info: CreateAndUpdateEntity, session: Session = Depends(get_db)):
